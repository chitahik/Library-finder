# LIBRARY FINDER V5.6 FINAL TITLE-AUTHOR
import os
import re
import time
import json
...
# -----------------------------
CHECKPOINT_VERSION = "v5.6-title-author-20260903"
...
def main():
    ...
    default_rows = pd.DataFrame(
        [{"도서명": "", "저자명": ""} for _ in range(10)]
    )
    book_table = st.data_editor(
        default_rows,
        hide_index=True,
        width="stretch",
        num_rows="dynamic",
        column_config={
            "도서명": st.column_config.TextColumn(
                "도서명",
                help="검색할 책 제목을 입력하세요.",
            ),
            "저자명": st.column_config.TextColumn(
                "저자명 (선택)",
                help="동일·유사 제목의 책을 구분할 때 입력하세요.",
            ),
        },
        key="book_input_table",
    )
    ...
    for _, row in book_table.iterrows():
        ...
        author = str(row.get("저자명", "") or "").strip()
    ...
    st.caption(
        "※ V5.6 FINAL · 위쪽 숫자는 '오늘 바로 빌릴 수 있는 제목 수 / 내가 입력한 전체 제목 수'입니다. 실제 소장 복본 수는 아래 책별 상세 결과에서만 표시합니다. "
