...
# -----------------------------
CHECKPOINT_VERSION = "v4.2-richest-jongno-largeprint-summary-20260902"
...
def _jongno_parse(html, title, library_key):
    """
    종로구 검색결과를 '책 한 권 카드' 단위로 끝까지 훑는다.
    일반판과 큰글자책을 분리하고 모든 복본의 대출상태를 집계한다.
    """
    soup = BeautifulSoup(html, "html.parser")

    # "바깥은 여름 김애란"처럼 저자까지 입력해도
    # 제목 부분을 찾을 수 있도록 여러 후보를 만든다.
    raw_words = re.sub(r"[!！?？:：·ㆍ]", " ", str(title or ""))
    raw_words = re.sub(r"\s+", " ", raw_words).strip().split()

    title_variants = []
    for nwords in range(len(raw_words), 0, -1):
        # 너무 짧은 1글자 단어 하나만으로는 매칭하지 않음
    ...
    # 상위 컨테이너가 검색결과 여러 권을 한꺼번에 포함하는 경우를 제거한다.
    # qualifying descendant가 있으면 부모는 버리고 가장 안쪽의 카드만 쓴다.
    cards = []
    for tag in soup.find_all(["li", "tr", "article", "div"]):
        if not qualifies(tag):
            continue
    ...
    if not cards:
        ...
        if any(v and v in whole for v in title_variants) and any(v and v in whole for v in lib_norms):
            return {
    ...
    for txt in cards:
        ...
        # '대출불가'/'대출중'이 있으면 절대 대출가능으로 세지 않는다.
...
def main():
    ...
    st.caption(
        "※ V4.2 VERIFIED · 종로구는 여러 검색어 결과 중 가장 많은 판본/복본을 찾은 결과를 사용하고, 큰글자책을 요약에도 표시합니다. "
