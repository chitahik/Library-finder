# LIBRARY FINDER V4.8.1 DIAGNOSTIC FIXED
import os
import re
import time
import json
...
# -----------------------------
CHECKPOINT_VERSION = "v4.8.1-raw-result-diagnostic-fix-20260903"
...
def fetch_jongno(title):
    ...
    for attempt in range(3):
        try:
            ...
            raw_records = []
            qnorm = norm(title)
            qwords = [norm(x) for x in re.sub(r"\s+", " ", title).strip().split()]
...
def main():
    ...
    st.caption(
        "※ V4.8.1 DIAGNOSTIC FIXED · V4.6의 정상 검색 요청은 그대로 두고, 실제 검색결과 링크와 부모 DOM 구조만 진단합니다. "
