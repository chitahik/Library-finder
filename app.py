...
def _find_and_check_library(page, library_name):
    """
    종로구 검색 페이지에서 특정 도서관 체크박스를 '반드시 체크된 상태'로 만든다.

    기존 버전은 label을 찾으면 무조건 click()했기 때문에,
    사이트에서 이미 체크된 항목은 오히려 해제될 수 있었다.
    """
    target = library_name.replace(" ", "")

    # 1) label의 for 속성으로 연결된 checkbox를 먼저 찾는다.
    ...
    for i in range(boxes.count()):
        ...
        if target in parent_txt.replace(" ", ""):
            try:
                if not box.is_checked():
                    box.check(force=True)
                return True
...
def _jongno_library_status(body, title, library_key):
    ...
    핵심:
    - '대출가능(비치중)' 또는 '비치중'은 즉시대출 가능으로 본다.
    - 다른 도서관 카드의 상태가 섞이지 않도록 도서관명 뒤쪽 결과 블록을 우선 본다.
    - 책은 찾았지만 상태 문구를 못 읽은 경우를 '즉시대출 없음'으로 단정하지 않는다.
    """
    ...
    for pat in lib_patterns:
        for m in re.finditer(pat, text):
            # 도서관명 '앞쪽'을 크게 포함하면 이전 카드의 대출상태가 섞일 수 있으므로
            # 도서관명 직전은 아주 조금만, 뒤쪽은 넉넉하게 잡는다.
            s = max(0, m.start() - 40)
    ...
    # 상태 문자열은 공백 차이를 무시해 판정
    normalized_snippets = [re.sub(r"\s+", "", s) for s in unique]

    # 명시적 대출불가 상태가 있더라도 같은 카드에 대출가능 복본이 있을 수 있으므로
    # '대출가능'/'비치중' 존재를 최우선으로 본다.
    available = any(
        ("대출가능" in s or "비치중" in s)
        and "대출가능0" not in s
        for s in normalized_snippets
    )
    ...
    explicitly_unavailable = any(
        any(marker in s for marker in unavailable_markers)
        for s in normalized_snippets
    )
    ...
    # 소장은 확인됐지만 상태 파싱이 안 된 경우.
    # 이것을 '못 빌림'으로 단정하면 false negative가 생긴다.
