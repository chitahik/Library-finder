# LIBRARY FINDER V4.8 RAW-RESULT DIAGNOSTIC
import os
import re
import time
import json
...
# -----------------------------
CHECKPOINT_VERSION = "v4.8-raw-result-boundary-diagnostic-20260902"
...
def fetch_jongno(title):
    ...
    for attempt in range(3):
        try:
            ...
            # V4.8: 실제 검색결과의 제목 링크 주변 구조를 그대로 관찰한다.
            # HTML 전체는 노출하지 않고, 검색어가 포함된 링크의 주변 텍스트/태그만 짧게 수집.
            raw_records = []
            qnorm = normalize_text(title)
            qwords = [normalize_text(x) for x in re.sub(r"\s+", " ", title).strip().split()]
            # 저자까지 입력된 경우 제목 앞 2단어도 탐색 키로 사용.
            title_probe = "".join(qwords[:2]) if len(qwords) >= 2 else qnorm

            for idx, a in enumerate(result_soup.find_all("a")):
                atxt = re.sub(r"\s+", " ", a.get_text(" ", strip=True)).strip()
                if not atxt:
                    continue
                natxt = normalize_text(atxt)
                if title_probe and title_probe not in natxt:
                    continue
                ...
                for depth in range(1, 7):
                    ...
                    ancestors.append({
                        ...
                        "has_cheongun": "청운문학도서관" in txt,
                        "has_bookcafe": ("청운효자동 북카페" in txt or "청운효자동북카페" in txt),
                        "has_large": ("큰글자" in txt or "대활자" in txt),
                        "has_available": ("대출가능" in txt or "비치중" in txt),
                        "has_unavailable": ("대출불가" in txt or "대출중" in txt),
                    })

                raw_records.append({
                    "link_index": idx,
                    "link_text": atxt[:250],
                    "href": str(a.get("href") or "")[:300],
                    "parent_tag": getattr(getattr(a, "parent", None), "name", ""),
                    "ancestors": ancestors,
                })
...
def _jongno_parse(html, title, library_key):
    """
    V4.5
    실제 종로구 페이지에서 제목 링크와 대출상태/도서관/청구기호가
    같은 <a> 안에 있지 않고 '형제 요소'로 분리되는 경우를 처리한다.

    책 제목을 포함한 요소에서 위로 올라가되,
    '청구기호 1개 + 대출상태 1개'를 가진 가장 작은 조상을
    책 1권의 결과 카드로 확정한다.
    """
    soup = BeautifulSoup(html, "html.parser")
    ...
    target_libs = [norm(x) for x in _jongno_names(library_key)]
    ...
    def title_match(txt):
        n = norm(txt)
        return any(v and v in n for v in title_variants)

    def target_library_match(txt):
        n = norm(txt)
        return any(v and v in n for v in target_libs)

    def state_count(txt):
        # '대출불가(대출중)'은 하나의 상태로 취급
        c = len(re.findall(r"대출(?:가능|불가)(?:\([^)]*\))?", txt))
        if c == 0:
            c = len(re.findall(r"(?:비치중|대출중)", txt))
        return c

    def call_count(txt):
        return len(re.findall(r"청구기호\s*[:：]?", txt))

    # 제목을 가진 작은 요소들을 출발점으로 삼는다.
    seeds = []
    for tag in soup.find_all(["a", "strong", "b", "span", "p", "dt", "dd", "li"]):
        txt = clean(tag)
        if txt and len(txt) <= 500 and title_match(txt):
            seeds.append(tag)
    ...
    for seed in seeds:
        ...
        # 제목 요소에서 최대 8단계까지만 올라간다.
        # '책 1권'을 나타내는 최소 컨테이너를 찾는 것이 목적.
        for _ in range(8):
            ...
            if (
                title_match(txt)
                and target_library_match(txt)
                and call_count(txt) == 1
                and state_count(txt) == 1
            ):
                chosen = node
                break
        ...
        if chosen is not None and id(chosen) not in seen_nodes:
            ...
            raw_cards.append(clean(chosen))

    # 제목 seed 방식이 실패한 특수 HTML에서는
    # 청구기호를 가진 가장 작은 카드만 보완한다.
    if not raw_cards:
        for tag in soup.find_all(["li", "tr", "article", "div"]):
            txt = clean(tag)
            if not (
            ...
            smaller = False
            for child in tag.find_all(["li", "tr", "article", "div"], recursive=True):
                if child is tag:
                    continue
                ctxt = clean(child)
                if (
                    title_match(ctxt)
                    and target_library_match(ctxt)
                    and call_count(ctxt) == 1
                    and state_count(ctxt) == 1
                ):
                    smaller = True
                    break
            if not smaller:
    ...
    if not raw_cards:
        ...
        if (
            any(v and v in whole for v in title_variants)
            and any(v and v in whole for v in target_libs)
        ):
            return {
                "status": "📚 소장 있음 / 대출상태 확인 필요",
                "available": 0, "copies": 1,
                "regular_total": 1, "regular_available": 0,
                "large_total": 0, "large_available": 0,
            }
        return {
            "status": "⚪ 소장 없음",
            "available": 0, "copies": 0,
            "regular_total": 0, "regular_available": 0,
            "large_total": 0, "large_available": 0,
        }
    ...
    for txt in raw_cards:
        ...
        call_match = re.search(
            r"청구기호\s*[:：]?\s*(.+?)(?=\s+(?:도서관|대출|자료위치|저자|출판사|발행연도)|$)",
            txt
        )
        call_no = norm(call_match.group(1)) if call_match else ""

        # 공식 표기의 '대출가능(비치중)'을 가장 강하게 신뢰한다.
        if "대출가능(비치중)" in compact:
            available = True
        elif "대출불가" in compact or "대출중" in compact:
            available = False
        ...
        key = (call_no or norm(txt), is_large)
        if key in seen:
            continue
        seen.add(key)
        cards.append((is_large, available))

    regular_total = sum(1 for large, _ in cards if not large)
    regular_available = sum(1 for large, avail in cards if not large and avail)
    large_total = sum(1 for large, _ in cards if large)
    large_available = sum(1 for large, avail in cards if large and avail)
    ...
    status = ("🟢 " if total_available else "🟡 ") + " · ".join(parts)
...
def main():
    ...
    with st.expander("🧪 종로구 실제 결과 구조 (V4.8)"):
        st.caption(
        ...
        for title in titles:
            ...
            for lib in LIBRARIES:
                ...
                raw_records = diag.get("raw_records") or []
                if raw_records:
                    st.markdown("**검색어가 들어간 실제 결과 링크와 부모 구조**")
                    for rec_i, rec in enumerate(raw_records, 1):
                        st.markdown(
                            f"**결과 링크 {rec_i}: {rec.get('link_text', '')}**"
                        )
                        st.caption(
                            f"href: {rec.get('href', '')} · "
                            f"직접 부모: <{rec.get('parent_tag', '')}>"
                        )

                        compact_rows = []
                        for anc in rec.get("ancestors", []):
                            flags = []
                            if anc.get("has_callno"): flags.append("청구기호")
                            if anc.get("has_cheongun"): flags.append("청운문학")
                            if anc.get("has_bookcafe"): flags.append("북카페")
                            if anc.get("has_large"): flags.append("큰글자")
                            if anc.get("has_available"): flags.append("대출가능")
                            if anc.get("has_unavailable"): flags.append("대출불가")

                            compact_rows.append(
                                f"D{anc.get('depth')} "
                                f"<{anc.get('tag')} class='{anc.get('class')}'> "
                                f"[{', '.join(flags) if flags else '-'}]\n"
                                f"{anc.get('text', '')}"
                            )

                        st.code("\n\n".join(compact_rows), language=None)
                else:
                    st.warning("검색어가 들어간 결과 링크를 찾지 못했습니다.")
    ...
    st.caption(
        "※ V4.8 RAW-RESULT DIAGNOSTIC · V4.6의 정상 검색 요청은 그대로 두고, 실제 검색결과 링크와 부모 DOM 구조만 진단합니다. "
