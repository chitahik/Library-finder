# LIBRARY FINDER V5.1 FINAL REGULAR-EDITION
import os
import re
import time
import json
...
# -----------------------------
CHECKPOINT_VERSION = "v5.0-final-regular-edition-20260903"
...
def _jongno_parse(html, title, library_key):
    """
    종로구 통합검색의 실제 반복 단위인 <article class="book-container">를
    책 1건으로 판독한다.

    종로구 검색은 여러 분관의 결과를 한 화면에 함께 반환하므로,
    각 article 안의 '도서관 :' 값을 확인해 목표 도서관만 남긴다.
    큰글자책/대활자본은 이 최종 버전에서 의도적으로 제외한다.
    """
    soup = BeautifulSoup(html, "html.parser")
    ...
    articles = soup.select("article.book-container")
    if not articles:
    ...
    for article in articles:
        ...
        # 같은 ISBN/상세링크를 공유하는 복본이 있어도 청구기호가 다르면 별도 1권이다.
        # 예: 813.7-김63ㅂ / 813.7-김63ㅂ=2
        # 따라서 href보다 목표 도서관 + 청구기호를 우선해 중복 제거한다.
        if call_no:
            key = (library_key, norm(call_no))
        elif href:
            key = (library_key, href)
        else:
...
def main():
    ...
    st.caption(
        "※ V5.1 FINAL · 종로구는 공식 통합검색 결과에서 해당 도서관의 일반판만 판독합니다. "
