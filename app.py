# LIBRARY FINDER V5.3 FINAL REGULAR-EDITION
import os
import re
import time
import json
...
def render_priority_summary(titles, detail):
    ...
    for rank, item in enumerate(summaries, start=1):
        ...
        availability_label = (
            "즉시대출 가능" if item["available_count"] > 0 else "즉시대출 없음"
        )
        st.markdown(
            f"**{medal} {item['name']}** — "
    ...
    st.caption(
        "※ 위 두 메뉴는 소장 권수나 분수 표기를 하지 않고, 입력한 책 제목 중 "
...
# -----------------------------
CHECKPOINT_VERSION = "v5.3-final-regular-edition-20260903"
...
def debug_jongno_fetch(title):
    """
    V5.3 디버그: 종로구 사이트에서 실제로 뭘 받아오는지 그대로 보여준다.
    - 어떤 form/action이 선택됐는지
    - 받은 HTML 안에 .book-container가 있는지
    - 받은 HTML 안에 도서관 이름/책 제목 문자열이 애초에 존재하는지
    이 세 가지만 보면 "AJAX로 나중에 채워지는 페이지인지"를 바로 알 수 있다.
    """
    st.markdown("### 🔧 종로구 사이트 원본 응답 디버그")
    try:
        html, result_url = fetch_jongno(title)
    except Exception as e:
        st.error(f"요청 자체가 실패했어요: {type(e).__name__}: {e}")
        return

    soup = BeautifulSoup(html, "html.parser")
    cards = soup.select(".book-container")

    st.write(f"**요청 URL:** {result_url}")
    st.write(f"**응답 길이:** {len(html)}자")
    st.write(f"**`.book-container` 개수:** {len(cards)}개")

    plain_hits = {
        "청운문학도서관 (문자열 자체)": "청운문학도서관" in html,
        norm(title) + " (정규화된 제목)": norm(title) in norm(html),
    }
    for label, found in plain_hits.items():
        st.write(f"**'{label}' 이 raw HTML에 존재:** {'✅ 있음' if found else '❌ 없음'}")

    if len(cards) == 0 and not plain_hits[norm(title) + " (정규화된 제목)"]:
        st.warning(
            "⚠️ 진단: raw HTML 안에 책 제목 자체가 없어요. "
            "이 페이지는 결과를 자바스크립트로 나중에 채우는 방식일 가능성이 높습니다. "
            "requests만으로는 그 결과를 가져올 수 없어요."
        )
    elif len(cards) == 0:
        st.warning(
            "⚠️ 진단: 제목 문자열은 있는데 `.book-container`가 안 잡혀요. "
            "실제 결과 카드의 CSS 클래스가 다른 것 같습니다."
        )
    else:
        st.success("✅ HTML 안에 결과 카드가 정상적으로 들어있어요. 파싱 로직 쪽을 더 봐야 해요.")

    with st.expander("원본 HTML 앞부분 3000자 보기"):
...
def main():
    ...
    with st.expander("🔧 디버그: 종로구 사이트 원본 응답 확인"):
        debug_title = st.text_input(
            "확인할 책 제목", value="안네의 일기", key="debug_title"
        )
        if st.button("디버그 실행", key="debug_run"):
            debug_jongno_fetch(debug_title)
    ...
    st.caption(
        "※ V5.3 FINAL · 종로구는 공식 통합검색 결과에서 해당 도서관의 일반판만 판독합니다. "
