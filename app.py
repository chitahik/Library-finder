...
# -----------------------------
CHECKPOINT_VERSION = "v4.4-anchor-only-when-available-20260902"
...
def _jongno_parse(html, title, library_key):
    """
    V4.4
    종로구 검색결과는 실제 '책 결과 링크(a)' 하나가 책 한 권이다.

    중요:
    - 실제 결과 링크가 하나라도 있으면 li/div 같은 부모 컨테이너는 절대 섞지 않는다.
    - 부모 컨테이너에는 다른 도서관/다른 복본 상태가 함께 들어갈 수 있어
      큰글자책·대출상태 오염이 생기기 때문이다.
    - 링크 결과가 전혀 없는 예외적인 HTML에서만 부모 카드 fallback을 쓴다.
    """
    soup = BeautifulSoup(html, "html.parser")
    ...
    # 1) 실제 종로구 검색결과 링크만 우선 수집.
    anchor_cards = []
    for a in soup.find_all("a"):
        txt = text_of(a)
        if qualifies_text(txt):
            anchor_cards.append(txt)

    # 실제 결과 링크가 있으면 그것만 사용한다.
    # 이게 V4.4의 핵심 수정.
    if anchor_cards:
        raw_cards = anchor_cards
    else:
        raw_cards = []

        # 2) 예외적인 HTML에서만 가장 안쪽의 부모 카드 fallback.
        for tag in soup.find_all(["li", "tr", "article", "div"]):
            txt = text_of(tag)
            ...
            for child in tag.find_all(["a", "li", "tr", "article", "div"], recursive=True):
                if child is tag:
                    continue
                if qualifies_text(text_of(child)):
    ...
    for txt in raw_cards:
        ...
        # 상태는 한 결과 링크 안에서만 판정.
        # '대출가능/비치중'이 명시되어 있으면 가능으로 우선 확정.
        explicitly_available = (
            "대출가능" in compact or "비치중" in compact
        )
        explicitly_unavailable = any(x in compact for x in (
            "대출불가", "대출중", "관외대출불가", "이용불가"
        ))

        # 같은 링크 안에 양쪽 문구가 비정상적으로 함께 있으면
        # 더 구체적인 '대출가능(비치중)' 조합을 우선한다.
        if "대출가능(비치중)" in compact:
            ...
            available = explicitly_available
            unavailable = False

        if call_no:
            key = (call_no, is_large)
        else:
            key = (norm(txt), is_large)
...
def main():
    ...
    st.caption(
        "※ V4.4 VERIFIED · 종로구는 실제 책 결과 링크만 우선 사용해 다른 도서관/다른 복본 상태가 섞이지 않도록 판독합니다. "
